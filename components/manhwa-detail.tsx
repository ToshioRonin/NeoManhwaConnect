...
import { Heart, Bell, BellRing, Calendar, User, Languages, BookOpen } from "lucide-react"
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div className="lg:col-span-1">
          <Card className="overflow-hidden border-border bg-card">
            <div className="aspect-[7/10] relative">
              <img src={manhwa.cover || "/placeholder.svg"} alt={manhwa.title} className="w-full h-full object-cover" />
            </div>
          </Card>
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                variant="outline"
                className={`border-border ${isFavorite ? "bg-primary/20 border-primary" : "bg-transparent"}`}
              ...
              <Button
                variant="outline"
                className={`border-border ${isFollowing ? "bg-primary/20 border-primary" : "bg-transparent"}`}
                onClick={() => setIsFollowing(!isFollowing)}
              >
                {isFollowing ? <BellRing className="w-4 h-4 mr-2 fill-primary" /> : <Bell className="w-4 h-4 mr-2" />}
                Seguir
              </Button>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Title and Metadata */}
          <div>
            <h1 className="text-4xl font-bold text-foreground mb-4">{manhwa.title}</h1>

            <div className="space-y-3">
              <div className="flex items-center gap-2 text-muted-foreground">
                <User className="w-4 h-4" />
                <span className="text-sm">Autor:</span>
                <span className="text-foreground font-medium">{manhwa.author}</span>
              </div>

              <div className="flex items-center gap-2 text-muted-foreground">
                <Languages className="w-4 h-4" />
                <span className="text-sm">Scan/Traductor:</span>
                <span className="text-foreground font-medium">{manhwa.translator}</span>
              </div>

              <div className="flex items-center gap-2 text-muted-foreground">
                <Calendar className="w-4 h-4" />
                <span className="text-sm">Próximo capítulo:</span>
                <span className="text-primary font-medium">{manhwa.nextChapter}</span>
              </div>
            </div>
          </div>
          ...
          {/* Genres */}
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">Géneros</h3>
          ...
          <div>
            ...
            <p className="text-muted-foreground leading-relaxed text-pretty">{manhwa.description}</p>
          </div>
          ...
          <Card className="p-4 border-border bg-card/50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-primary">{manhwa.totalChapters}</p>
              </div>
      ...
      {/* Chapter List */}
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-4">Listado de Capítulos</h2>
