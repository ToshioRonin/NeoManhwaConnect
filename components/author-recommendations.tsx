...
import { UserPlus, CheckCircle2, Pen, Paintbrush, Languages } from "lucide-react"
      ...
      <ScrollArea className="h-[calc(100vh-4rem)]">
        <div className="p-4">
          <Card className="p-4 bg-background border-border">
            <h2 className="font-bold text-lg text-foreground mb-4">Mis recomendaciones</h2>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <Button
                ...
                className={selectedFilter === "author" ? "bg-primary text-primary-foreground" : ""}
              >
              ...
              <Button
                ...
                className={selectedFilter === "artist" ? "bg-primary text-primary-foreground" : ""}
              >
              ...
              <Button
                ...
                className={selectedFilter === "scan" ? "bg-primary text-primary-foreground" : ""}
              >
              ...
              {filteredAuthors.map((author) => (
                <div key={author.id} className="flex items-start gap-3">
                  <Avatar className="w-12 h-12">
                    ...
                    <AvatarFallback className="bg-muted text-muted-foreground">{author.name[0]}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1">
                      <p className="font-semibold text-sm text-foreground truncate">{author.name}</p>
                      {getTypeIcon(author.type)}
                      {author.verified && <CheckCircle2 className="w-4 h-4 text-primary flex-shrink-0" />}
                    </div>
                    <p className="text-xs text-muted-foreground truncate">
                      {author.username} · {getTypeLabel(author.type)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">{author.works}</p>
                    <p className="text-xs text-muted-foreground">{author.followers} seguidores</p>
                    <Button size="sm" className="mt-2 w-full bg-primary text-primary-foreground hover:bg-[#9D77FF]">
